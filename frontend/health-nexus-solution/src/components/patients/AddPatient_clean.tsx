...
const AddPatient = () => {
  ...
  const handleTemplateSave = () => {
    // Prevent saving if all fields are empty for SOAP Note or Blank
    if (selectedTemplate === "SOAP Note") {
      const fields = ["subjective", "objective", "assessment", "plan"];
      const allEmpty = fields.every((field) => !templateData[field]?.trim());
      if (allEmpty) {
        toast({
          title: "Validation Error",
          description: "Please fill in at least one field before saving the SOAP Note template.",
          variant: "destructive",
        });
        return;
      }
    ...
    if (selectedTemplate === "Blank") {
      if (!templateData.notes?.trim()) {
        toast({
          title: "Validation Error",
          description: "Please enter notes before saving the Blank template.",
          variant: "destructive",
        });
        return;
      }
  ...
  const [showCancelDialog, setShowCancelDialog] = useState(false);  const handleSaveAll = async () => {
    ...
    if (!isDoctor && !isReceptionist) {
      ...
      return;
    }setLoading(true);    try {      // Send only the fields expected by the backend (no id or registrationDate)
      const patientData = {
        name: form.name,
        email: form.email,
        phone: form.phone,
  ...
  // Add this utility function inside your component or above it
...
>
          ...
          <Card className="h-[82vh] flex flex-col">              <CardHeader className="border-b">
                ...
                <nav className="flex justify-around items-center">
                  <ul className="flex gap-10 text-base">
                    <li>
                      <button
                          onClick={() => { setActiveTab('personal'); scrollTo(personalRef); }}
                          className={`hover:text-[#79c942] text-1xl font-bold ${
                          activeTab === 'personal'
                            ? 'text-[#79c942] underline underline-offset-8 decoration-2 decoration-[#79c942]'
                            : ''
                    ...
                    {isDoctor && (
                      <>
                        <li>
                          <button
                              onClick={() => { setActiveTab('exam'); scrollTo(examRef); }}
                              className={`hover:text-[#79c942] text-1xl font-bold ${
                              activeTab === 'exam'
                                ? 'text-[#79c942] underline underline-offset-8 decoration-2 decoration-[#79c942]'
                                : ''
                        ...
                        <li>
                          <button
                              onClick={() => { setActiveTab('history'); scrollTo(historyRef); }}
                              className={`hover:text-[#79c942] text-1xl font-bold ${
                              activeTab === 'history'
                                ? 'text-[#79c942] underline underline-offset-8 decoration-2 decoration-[#79c942]'
                                : ''
              ...
              </CardHeader>            <CardContent className="flex-1 overflow-y-auto space-y-10 py-6">
                {isReceptionist && (
                ...
                <div ref={personalRef} className="border border-[#79c942] p-4 rounded">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        className="focus-visible:ring-[#79c942] focus-visible:border-[#79c942]"
                      />
                    </div>
                    <div>
                      ...
                      >
                        <SelectTrigger className="focus-visible:ring-2 focus-visible:!ring-[#79c942] focus-visible:!border-[#79c942]">
                          <SelectValue placeholder="Select gender" />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem
                            value="male"
                            className="data-[highlighted]:bg-[#79c942] data-[highlighted]:text-white cursor-pointer"
                          >
                            Male
                          </SelectItem>
                          <SelectItem
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={e => {
                          ...
                          const computedAge = computeAge(e.target.value);
                          if (computedAge) {
                            handleChange('age', computedAge);
                          }
                        }}
                        className="focus-visible:ring-[#79c942] focus-visible:border-[#79c942]"
                      />
                ...
                {isDoctor && (
                  <>                    <div ref={examRef} className="border border-[#79c942] p-4 rounded">
                      ...
                      <div className="mt-6">
                      {/* 1. Divider line before Physical Examination Templates */}
                      <hr className="border-t-2 border-[#79c942] mb-6" />
                        ...
                        <div className="flex items-center gap-4 max-w-xs mb-4">
                          <div className="flex-1">
                            ...
                            <Select
                              ...
                              <SelectTrigger className="focus:ring-[#79c942] focus:border-[#79c942]">
  ...
  className="bg-[#79c942] hover:bg-[#79c942]/90 text-white"
>
  Add
</Button>
                      )}
                      ...
                      {showForm && selectedTemplate === 'SOAP Note' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {['subjective', 'objective', 'assessment', 'plan'].map((field, idx) => {
                          ...
                          <div className="col-span-1 md:col-span-2 flex justify-end gap-2">
                            <Button variant="outline" className="hover:bg-[#79c942] hover:text-white" onClick={() => setShowCancelDialog(true)}>
        Cancel
      </Button>
                            <Button
  ...
  Save
                ...
                {showForm && selectedTemplate === 'Blank' && (
                    <div>
                      <Label>Title</Label>
                      <Input
                        value={templateData.notesTitle || ''}
                        onChange={e => handlePrescriptionChange('notesTitle', e.target.value)}
                        placeholder="Enter notes title (e.g. My findings, Progress Note, etc.)"
                        className="focus-visible:rinxg-[#79c942] focus-visible:border-[#79c942] mb-2"
                      />
                      <Label>Notes</Label>
                      <Textarea
                      ...
                      <div className="flex justify-end gap-2 mt-2">
                        <Button
                          variant="outline"
                          className="hover:bg-[#79c942] hover:text-white"
  ...
  <Button
    ...
    className={
      currentPrescriptionTab === tab
        ? 'bg-[#79c942] text-white hover:bg-[#79c942] hover:text-white'
        : 'hover:bg-[#79c942]/80 hover:text-white'
    }
  >
    {tab}
  </Button>
    ...
    <div className="flex gap-2">
      <label className="flex items-center gap-1 cursor-pointer">
        <input
          type="radio"
          name="nameType"
          checked={templateData.nameType === 'Generic'}
          onChange={() => handlePrescriptionChange('nameType', 'Generic')}
          className="accent-[#79c942] focus:ring-[#79c942]"
        />
        <span className="text-black font-medium">Generic Name</span>
      </label>
      <label className="flex items-center gap-1 cursor-pointer">
        <input
          type="radio"
          name="nameType"
          checked={templateData.nameType === 'Brand'}
          onChange={() => handlePrescriptionChange('nameType', 'Brand')}
          className="accent-[#79c942] focus:ring-[#79c942]"
        />
    ...
    <Input
      ...
      className={`${prescriptionErrors.name ? "border-red-500" : ""} focus-visible:ring-[#79c942] focus-visible:border-[#79c942]`}
    ...
    <Input
      ...
      className={`${prescriptionErrors.dose ? "border-red-500" : ""} focus-visible:ring-[#79c942] focus-visible:border-[#79c942]`}
    ...
    <Input
      ...
      className={`${prescriptionErrors.quantity ? "border-red-500" : ""} focus-visible:ring-[#79c942] focus-visible:border-[#79c942]`}
    ...
    >
      ...
      <SelectContent>
        <SelectItem value="Once daily" className="data-[highlighted]:bg-[#79c942] data-[highlighted]:text-white cursor-pointer">Once daily</SelectItem>
        <SelectItem value="Twice daily" className="data-[highlighted]:bg-[#79c942] data-[highlighted]:text-white cursor-pointer">Twice daily</SelectItem>
        <SelectItem value="Every 8 hours" className="data-[highlighted]:bg-[#79c942] data-[highlighted]:text-white cursor-pointer">Every 8 hours</SelectItem>
        <SelectItem value="custom" className="data-[highlighted]:bg-[#79c942] data-[highlighted]:text-white cursor-pointer">Custom</SelectItem>
      </SelectContent>
    </Select>
    {templateData.frequency === 'custom' && (
      <Input
        className={`mt-2 focus-visible:ring-[#79c942] focus-visible:border-[#79c942] ${prescriptionErrors.customFrequency ? "border-red-500" : ""}`}
    ...
    <Input
      type="date"
      className={`${prescriptionErrors.startDate ? "border-red-500" : ""} focus-visible:ring-[#79c942] focus-visible:border-[#79c942]`}
    ...
    <Input
      type="date"
      className={`${prescriptionErrors.endDate ? "border-red-500" : ""} focus-visible:ring-[#79c942] focus-visible:border-[#79c942]`}
    ...
    <Textarea
      ...
      className={`${prescriptionErrors.notes ? "border-red-500" : ""} focus-visible:ring-[#79c942] focus-visible:border-[#79c942]`}
      value={templateData.notes || ''}
      onChange={(e) => handlePrescriptionChange('notes', e.target.value)}
    />
    <div className="flex justify-end gap-2 mt-2">
...
{currentPrescriptionTab === 'Brand' && (
  ...
  {templates.some(item => item.type === 'SOAP Note') && templates
    ...
    .map((item, idx) => (
      <AccordionItem
        key={`soap-${idx}`}
        value={`soap-${idx}`}
        className="relative" // remove bg-[#79c942]/20
      >
        <AccordionTrigger>
          <span className="text-[#79c942] font-bold">SOAP Note</span>
        </AccordionTrigger>
        <div className="absolute right-4 top-3 flex gap-2">
          <Pencil className="text-[#FF6B6B] cursor-pointer hover:text-[#79c942]" onClick={() => handleEditTemplate(idx)} />
          <Trash2 className="text-[#FF6B6B] cursor-pointer hover:text-[#79c942]" onClick={() => { setDeleteIndex(idx); setShowDeleteConfirm(true); }} />
        </div>
        <AccordionContent>
  ...
  {templates.some(item => item.type === 'E-Prescription') && (
    <AccordionItem value="e-prescription" className="relative">
      <AccordionTrigger>
        <span className="text-[#79c942] font-bold">E-Prescription</span>
      </AccordionTrigger>
      <AccordionContent>
  ...
  {templates.some(item => item.type === 'Blank') && templates
    ...
    .map((item, idx) => (
      ...
      >
        <AccordionTrigger>
          <span className="text-[#79c942] font-bold flex items-center gap-2">
            {item.data.notesTitle || 'Blank'}
            <Pencil
              size={16}
              className="text-[#79c942] cursor-pointer hover:text-[#4e7d1e]"
              onClick={e => {
                e.stopPropagation();
        ...
        <AccordionContent>
          <div><strong>Notes:</strong> {item.data.notes}</div>
              ...
              <div ref={historyRef} className="border border-[#79c942] p-4 rounded">
                  ...
                  {['illnesses', 'surgeries', 'allergies', 'medications', 'familyHistory', 'socialHistory'].map((field) => (
                    <label key={field} className="flex items-center gap-2">
                      <input
                        ...
                        className="focus:ring-[#79c942] focus:border-[#79c942] data-[state=checked]:bg-[#79c942]"
                      />
              ...
              <DialogFooter>
                <Button
                  ...
                  onClick={() => {
                    ...
                    setShowForm(false); // Close the template form
                ...
                <Button
                  ...
                  className="hover:bg-[#79c942] hover:text-white"
                  onClick={() => {
                    setShowCancelDialog(false); // Just close the modal, keep template open
                  }}
                >
                  No
                </Button>
