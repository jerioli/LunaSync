...
const AdminDashboard = () => {
    ...
    const fetchInventory = async () => {
      try {
        ...
        if (response.data.success) {
          setInventoryCount(Array.isArray(response.data.data) ? response.data.data.length : 0);
  ...
  useEffect(() => {
    const fetchStaff = async () => {
      try {
        ...
        const staffMembers = response.data.filter(user => user.role !== 'patient');
  ...
  const patientRegistrationsData = (() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const count = localPatients.filter(patient => {
        ...
        const dateField = patient.created_at || patient.date_registered || patient.dateRegistered || patient.created || patient.registration_date;
        ...
        try {
          ...
          const matches = patientDate.getMonth() === monthIndex && patientDate.getFullYear() === year;
          
          if (matches) {
            console.log(`Patient registered in ${monthNames[monthIndex]} ${year}:`, patient.id, dateField);
          }
  ...
  const completedAppointments = appointments
    ...
    .map(apt => {
      ...
      const patient = localPatients.find(p => String(p.id) === String(apt.patient));
      
      return {
        id: apt.id,
        patientName: getPatientName(apt.patient, apt),
        age: patient?.age || patient?.date_of_birth ? 
          (patient?.age || new Date().getFullYear() - new Date(patient.date_of_birth).getFullYear()) : 'N/A',
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Welcome back, Dr. Admin! Here's your clinic overview.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-green-600 mt-1">+18 new this week</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">All on duty today</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <Card className="shadow-md hover:shadow-lg transition-shadow">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="mt-1">Monthly patient registration trends</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart 
                  data={patientRegistrationsData}
                >
                  <CartesianGrid 
                    strokeDasharray="3 3" 
                    stroke="#e2e8f0"
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">Appointment Type</th>
                ...
                {completedAppointments.length > 0 ? (
                  completedAppointments.map((apt) => (
                    <tr key={apt.id} className="border-b hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-sm text-blue-600">{apt.appointmentType}</td>
                      <td className="py-3 px-4 text-sm text-blue-600">{apt.doctorName}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
