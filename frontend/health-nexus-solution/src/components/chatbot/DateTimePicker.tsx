...
const DateTimePicker: React.FC<DateTimePickerProps> = ({
  ...
  useEffect(() => {
    const dateToUse =
      currentSelectedDate ||
      (timeOnlyMode && selectedDate ? selectedDate : null);
    if (dateToUse) {
  ...
  return (
    <div
      className="bg-white rounded-lg border border-gray-200 p-3 sm:p-4 w-full max-w-2xl mx-auto relative min-w-0"
      style={{ zIndex: 10 }}
    >
      {/* Header */}
          ...
          <div className="relative min-w-0">
            ...
            <TimePopover
              ...
              disabled={
                disabled ||
                loadingTimeSlots ||
                (!currentSelectedDate && !timeOnlyMode)
              }
            />
          </div>
        )}
        ...
        timeSlots.length > 0 && (
          <div className="text-xs text-gray-500 mb-4">
            {loadingTimeSlots ? (
              ...
              <div>
                <span className="text-[#79c942]">
                  {timeSlots.filter((s) => s.available && !s.booked).length}{" "}
                  available
                </span>
              </div>
            )}
