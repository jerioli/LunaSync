...
export const ChatForm = ({
  ...
  const handleInputChange = useCallback(
    (name: string, value: string, fieldType?: string) => {
      // Enforce phone number max length of 11 digits
      if (fieldType === "tel") {
        const digitsOnly = value.replace(/\D/g, "");
        if (digitsOnly.length > 11) {
          return; // Don't update if exceeds 11 digits
        }
        value = digitsOnly; // Store only digits
      }
  ...
  const memoizedSelectOptions = useMemo(() => {
    const selectFieldsOptions: Record<
      string,
