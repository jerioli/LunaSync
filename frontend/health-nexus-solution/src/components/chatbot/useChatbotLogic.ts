...
⚠️ For Emergency, Please contact clinic directly via call: ${clinicPhone}`;
  ...
  const handleProfanityDetection = (userInput: string): boolean => {
    ...
    if (containsProfanity(userInput)) {
      addBotMessage('Please keep our conversation respectful and professional.');
      return true;
    }
    return false;
  };
  ...
  const handleSendMessage = async () => {
    ...
    if (chatMode === 'appointment') {
      if (chatStep === 2) {
        ...
        setTimeout(() => {
          addBotMessage('Please select your sex', [
        ...
        setTimeout(() => {
          addBotMessage('Here is your appointment summary');
