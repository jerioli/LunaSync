...
import { Bell, Calendar, CheckSquare, Clock, FileText, LogOut, Pill, Settings, UserCheck } from 'lucide-react';
...
export const TopBar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const filteredNotifications = activeTab === 'unread' 
      ? notifications.filter(n => n.isUnread && !readNotifications.has(n.id))
      : notifications.map(n => ({
          ...n,
    ...
    if (currentPage === 1) {
      ...
      const firstPageNotifications = filteredNotifications.slice(0, NOTIFICATIONS_PER_PAGE);
      ...
      const newNotifications = filteredNotifications.slice(startIndex, endIndex);
    ...
    const currentlyLoaded = Math.min(currentPage * NOTIFICATIONS_PER_PAGE, totalAvailable);
    setHasMoreNotifications(currentlyLoaded < totalAvailable);
  }, [notifications, activeTab, readNotifications, currentPage]);

  const mapNotifications = (appointmentData, medCertData = [], prescriptionData = []) => {
    const notifications = [];
    ...
    if (isReceptionist) {
      ...
      if (Array.isArray(appointmentData)) {
        appointmentData
          ...
          .forEach(appt => {
            ...
            if (appt.notes && appt.notes.includes('Patient Details (Pending):')) {
              try {
                const patientDetails = JSON.parse(appt.notes.split('Patient Details (Pending):')[1].trim());
      ...
      if (Array.isArray(medCertData)) {
        medCertData
          .filter(cert => cert.status === 'pending' || cert.status === 'submitted')
          .forEach(cert => {
            notifications.push({
              id: `medcert-${cert.id}`,
      ...
      if (Array.isArray(prescriptionData)) {
        prescriptionData
          .filter(presc => presc.status === 'pending' || presc.status === 'submitted')
          .forEach(presc => {
            notifications.push({
              id: `prescription-${presc.id}`,
              type: 'prescription_request',
              title: 'Prescription Request',
              message: `From: ${presc.patient_name || 'Unknown Patient'} - ${presc.medication_name}`,
      ...
      if (Array.isArray(appointmentData)) {
        appointmentData
          .filter(appt => appt.status === 'approved' || appt.status === 'checked_in' || appt.status === 'ongoing')
          .forEach(appt => {
            ...
            notifications.push({
              id: `appointment-${appt.id}`,
              type: isCheckedIn ? 'patient_checkedin' : isOngoing ? 'appointment_ongoing' : 'appointment_approved',
              title: isCheckedIn ? 'Patient Checked In' : isOngoing ? 'Appointment Ongoing' : 'Appointment Approved',
              message: `Patient: ${patientName}${isCheckedIn ? ' is ready for consultation' : isOngoing ? ' appointment is now in progress' : ''}`,
              time: getRelativeTime(appt.updated_at || appt.created_at || new Date()),
              date: appt.date,
              appointmentTime: appt.time,
              appointmentType: appt.appointment_type,
              icon: isCheckedIn ? UserCheck : isOngoing ? Clock : CheckSquare,
              iconColor: isCheckedIn ? '#059669' : isOngoing ? '#f97316' : '#3b82f6',
      ...
      if (Array.isArray(medCertData)) {
        medCertData
          .filter(cert => cert.status === 'approved' || cert.status === 'ready' || cert.status === 'receptionist_approved')
          .forEach(cert => {
            const isReceptionistApproved = cert.status === 'receptionist_approved';
            notifications.push({
              id: `medcert-${cert.id}`,
              type: isReceptionistApproved ? 'medcert_receptionist_approved' : 'medcert_approved',
              title: isReceptionistApproved ? 'Medical Certificate Needs Review' : 'Medical Certificate Ready',
              message: `For: ${cert.patient_name || 'Unknown Patient'}${isReceptionistApproved ? ' - Approved by receptionist, awaiting doctor review' : ''}`,
              time: getRelativeTime(cert.updated_at || cert.created_at || new Date()),
      ...
      if (Array.isArray(prescriptionData)) {
        prescriptionData
          .filter(presc => presc.status === 'approved' || presc.status === 'ready' || presc.status === 'receptionist_approved')
          .forEach(presc => {
            const isReceptionistApproved = presc.status === 'receptionist_approved';
            notifications.push({
              id: `prescription-${presc.id}`,
              type: isReceptionistApproved ? 'prescription_receptionist_approved' : 'prescription_approved',
              title: isReceptionistApproved ? 'Prescription Needs Review' : 'Prescription Ready',
              message: `For: ${presc.patient_name || 'Unknown Patient'} - ${presc.medication_name}${isReceptionistApproved ? ' - Approved by receptionist, awaiting doctor review' : ''}`,
              time: getRelativeTime(presc.updated_at || presc.created_at || new Date()),
    ...
    return notifications.sort((a, b) => new Date(b.data.created_at || b.data.updated_at || new Date()).getTime() - new Date(a.data.created_at || a.data.updated_at || new Date()).getTime());
  };
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'totalNotificationCount' || e.key === 'unreadNotificationCount') {
        updateNotificationCount();
      }
    };
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        try {
          const prescriptionResponse = await axios.get('/prescription-requests/');
          prescriptionData = prescriptionResponse.data;
        } catch (error) {
          console.warn('Could not fetch prescription requests:', error);
        }
        
        console.log('Fetched data:', { appointmentsData, medCertData, prescriptionData });
        
        if (Array.isArray(appointmentsData)) {
          const mappedNotifications = mapNotifications(appointmentsData, medCertData, prescriptionData);
          setNotifications(mappedNotifications);
          
          // Update counts
          const totalCount = mappedNotifications.length;
          const actualUnreadCount = mappedNotifications.filter(n => n.isUnread && !readNotifications.has(n.id)).length;
          ...
          localStorage.setItem('unreadNotificationCount', actualUnreadCount.toString());
        } else {
          console.error('Invalid appointments response format:', appointmentsData);
          setNotifications([]);
        }
      } catch (error) {
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="relative notification-dropdown">
            ...
            <Button 
              ...
              title={`${totalNotificationCount > 0 ? `${totalNotificationCount} notification${totalNotificationCount > 1 ? 's' : ''}${unreadCount > 0 ? ` (${unreadCount} unread)` : ''}` : 'No notifications'}`}
              style={{ 
                color: colors.primaryColor,
                    ...
                    (() => {
                      ...
                      return (
                        <>
                                    ...
                                    <div className="flex items-start justify-between">
                                      <div className="flex-1">
                                        <p className={`text-sm leading-tight ${
                                          showAsUnread ? 'font-semibold text-gray-900' : 'font-medium text-gray-700'
                                        }`}>
                                          {notification.title}
                                        </p>
                                        <p className={`text-sm mt-1 leading-tight ${
                                          showAsUnread ? 'text-gray-700' : 'text-gray-500'
                                        }`}>
                                          {notification.message}
                                        </p>
                                        
                                        {/* Additional details for appointments */}
                                        {(notification.date || notification.appointmentTime) && (
                          ...
                          {!hasMoreNotifications && displayedNotifications.length > NOTIFICATIONS_PER_PAGE && (
                            <div className="text-center py-4">
                              <p className="text-xs text-gray-400">You've seen all notifications</p>
                            </div>
                          )}
                        </>
                      );
